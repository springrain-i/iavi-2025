...
def live_project(args):
	...
	os.makedirs(args.outdir, exist_ok=True)
	frame_idx = 0
	print('按 q 或 Esc 退出，按 s 保存当前帧，按 r 切换录制')

	# prepare recording variables
	recording = False
	writer = None
	# video output path: project root / data / video / live.mp4
	project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
	video_dir = os.path.join(project_root, 'data', 'video')
	os.makedirs(video_dir, exist_ok=True)
	video_path = os.path.join(video_dir, 'live.mp4')
	# preferred codec for mp4
	fourcc = cv2.VideoWriter_fourcc(*'mp4v')
	# try to get camera FPS, fallback to 20
	cam_fps = cap.get(cv2.CAP_PROP_FPS)
	if cam_fps is None or cam_fps <= 0 or np.isnan(cam_fps):
		cam_fps = 20.0
	...
	while True:
		...
		if key == ord('q') or key == 27:
			...
			if recording:
				...
				writer = cv2.VideoWriter(video_path, fourcc, float(cam_fps), (w, h))
				if not writer or not writer.isOpened():
					print('无法创建视频写入器，请检查编解码器/路径')
					writer = None
					recording = False
				else:
					print('开始录制 ->', video_path)
			else:
...
def main():
	...
	parser.add_argument('--scale', type=float, default=1.0, help='uniform scale factor to apply to point cloud before projection (about its centroid)')
