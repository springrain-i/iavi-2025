...
def load_ply_with_colors(ply_file, max_points=100000):
    """加载点云，包含颜色信息"""
...
def main_comparison():
    """比较不同剔除方法的效果"""
    projector = ImprovedMultiProjector('camera_calibration.npz')
    points_3d, colors = load_ply_with_colors('cat.ply')
    points_3d, colors = simple_scale_pointcloud(points_3d, colors)
    points_3d[:, 2] = -points_3d[:, 2]
    bgr_colors = colors_to_bgr(colors)

    # 查找图像
    file_path = 'chessboard_1_one/*.png'
    image_files = glob.glob(file_path)
    ...
    # 只处理第一张图像进行演示
    img_id, img_path = list(image_mapping.items())[0]
    ...
    for method, method_name in methods:
        ...
        points_2d, visible_colors, valid_indices = projector.project_points(
            points_3d, colors, img_id,
            method=method,
        ...
        if len(points_2d) == 0:
            print(f"  {method_name}: 无点被投影")
            continue

        print(f"  {method_name}: 投影 {len(points_2d)} 点")
        ...
        for i, pt in enumerate(points_2d):
            ...
            if 0 <= x < result.shape[1] and 0 <= y < result.shape[0]:
                color = tuple(map(int, bgr_colors[valid_indices[i]]))
                cv2.circle(result, (x, y), 1, color, -1)

        # 保存结果
        output_file = os.path.join(output_dir, f"result_{img_id}_{method}.jpg")
        cv2.imwrite(output_file, result)
        print(f"  保存: {output_file}")


def main_simple_solution():
    """
    最简单的解决方案：只进行基础剔除，但按深度排序绘制
    这样近处的点会覆盖远处的点，形成自然的遮挡关系
    """
    projector = ImprovedMultiProjector('camera_calibration.npz')
    points_3d, colors = load_ply_with_colors('cat.ply')
    points_3d, colors = simple_scale_pointcloud(points_3d, colors)
    ...
    for img_id, img_path in image_mapping.items():
        ...
        # 只进行基础剔除，不进行背面剔除
        points_2d, visible_colors, valid_indices = projector.project_points(
            points_3d, colors, img_id, method="no_culling"
        )
        ...
        print(f"  投影 {len(points_2d)} 点")

        # 按深度从远到近绘制（近处的点覆盖远处的点）
        result = image.copy()

        # 获取深度信息并排序
        params = projector.image_params[img_id]
        R = projector.rodrigues_to_matrix(params['rvec'])
        points_cam = (R @ points_3d[valid_indices].T + params['tvec']).T
        depths = points_cam[:, 2]

        # 按深度从远到近排序
        sorted_indices = np.argsort(-depths)  # 负号表示从远到近

        for idx in sorted_indices:
            pt = points_2d[idx]
            x, y = int(pt[0]), int(pt[1])
            if 0 <= x < result.shape[1] and 0 <= y < result.shape[0]:
                color = tuple(map(int, bgr_colors[valid_indices[idx]]))
                cv2.circle(result, (x, y), 1, color, -1)

        output_file = os.path.join(output_dir, f"result_{img_id}.jpg")
        cv2.imwrite(output_file, result)
        print(f"  保存: {output_file}")


def main_best_solution():
    """
    最佳解决方案：结合深度排序和适度的可见性剔除
    """
    projector = ImprovedMultiProjector('camera_calibration.npz')
    points_3d, colors = load_ply_with_colors('cat.ply')
    points_3d, colors = simple_scale_pointcloud(points_3d, colors)
    ...
    for img_id, img_path in image_mapping.items():
        ...
        # 使用适度的可见性剔除（角度阈值较大，不会过度剔除）
        points_2d, visible_colors, valid_indices = projector.project_points(
            points_3d, colors, img_id,
            method="visibility",
            max_angle=120  # 较大的角度阈值，保留更多点
        )
...
if __name__ == "__main__":
    ...
    # 方法2: 最简单的解决方案（推荐）
    main_simple_solution()

    # 方法3: 最佳解决方案（适度的剔除+深度排序）
    # main_best_solution()